...
//! ACE_TemplateFrameJobScheduler<SCR_ChimeraCharacter, ACE_Medical_CharacterContext, ACE_Medical_Medication_SimulationJob>
//! as workaround for:
//! - https://feedback.bistudio.com/T193122
//! - https://feedback.bistudio.com/T193389
[BaseContainerProps()]
class ACE_Medical_Medication_JobScheduler : ACE_IFrameJob
{
	...
	protected ref ACE_Medical_Medication_SimulationJob m_pTemplateJob;
	protected ref array<ref ACE_Medical_Medication_SimulationJob> m_aJobs = {};
	...
	void OnInit(ACE_Medical_Medication_SimulationJob templateJob)
	{
	...
	protected void UpdateQueueStep(float currentTime)
	{
		...
		{
			ACE_Medical_Medication_SimulationJob job = m_aJobs[i];
	...
	protected void UpdateObjectsStep(float currentTime)
	{
		...
		{
			ACE_Medical_Medication_SimulationJob job = m_aJobs[m_iCurrentJobIdx];
	...
	void Register(SCR_ChimeraCharacter object)
	{
		...
		ACE_Medical_Medication_SimulationJob job = CreateJob();
	...
	{
		foreach (int i, ACE_Medical_Medication_SimulationJob job : m_aJobs)
		{
	...
	{
		ACE_Medical_Medication_SimulationJob job = m_aJobs[machineIdx];
		m_aJobs.Remove(machineIdx);
		job.OnStop();
	}
	
	//------------------------------------------------------------------------------------------------
	protected ACE_Medical_Medication_SimulationJob CreateJob()
	{
		ACE_Medical_Medication_SimulationJob instance = new ACE_Medical_Medication_SimulationJob();
