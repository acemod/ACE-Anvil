...
class ACE_Medical_RespiratorySystem : ACE_Medical_BaseSystem3
{
	...
	protected void UpdatePaCO2(ACE_Medical_RespiratoryComponent component, float timeSlice)
	{
		...
		float paCO2Rate;
		if (ACE_Math.IsClose(ventRatio, 1, atol: 1e-3))
			paCO2Rate = Math.Clamp(m_Settings.m_fDefaultPaCO2KPA - component.GetPaCO2() + 150 * Math.Max(component.GetAnaerobicPressure() - 1, 0), -m_Settings.m_fMaxPaCO2RateKPAPS, m_Settings.m_fMaxPaCO2RateKPAPS);
		else if (ventRatio > 1)
	...
	protected void UpdatePaO2(ACE_Medical_RespiratoryComponent component, SCR_CharacterDamageManagerComponent damageManager, float timeSlice)
	{
		...
		float palvO2 = Math.Max(fiO2 * (101.325 - 6.266) - component.GetPaO2() / component.GetAnaerobicPressure(), 0.133);
