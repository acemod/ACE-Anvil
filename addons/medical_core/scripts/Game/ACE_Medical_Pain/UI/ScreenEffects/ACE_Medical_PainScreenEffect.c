...
class ACE_Medical_PainScreenEffect : SCR_BaseScreenEffect
{
	...
	// Widgets
	protected ImageWidget m_wSimplePainEffect;
	...
	{
		m_wSimplePainEffect = ImageWidget.Cast(m_wRoot.FindAnyWidget("ACE_Medical_SimplePainEffect"));
	}	
	...
	{
		if (!m_wSimplePainEffect || !m_pPainHZ)
			return;
		...
		m_wSimplePainEffect.SetSaturation(1);
		
		if (isFadeIn)
		{
			AnimateWidget.Opacity(m_wSimplePainEffect, effectStrength, PAINEFFECT_OPACITY_FADEIN_1_DURATION);
			AnimateWidget.AlphaMask(m_wSimplePainEffect, effectStrength * 0.5, PAINEFFECT_PROGRESSION_FADEIN_1_DURATION);
		}
		else
		{
			AnimateWidget.Opacity(m_wSimplePainEffect, 0, PAINEFFECT_OPACITY_FADEOUT_2_DURATION);
			AnimateWidget.AlphaMask(m_wSimplePainEffect, 0, PAINEFFECT_PROGRESSION_FADEOUT_2_DURATION);
		}
	...
	{
		AnimateWidget.Opacity(m_wSimplePainEffect, 0, PAINEFFECT_PROGRESSION_FADEOUT_1_DURATION);
		AnimateWidget.AlphaMask(m_wSimplePainEffect, 0, PAINEFFECT_OPACITY_FADEOUT_1_DURATION);
	...
	override void ClearEffects()
	{
		...
		if (m_wSimplePainEffect)
		{
			AnimateWidget.StopAllAnimations(m_wSimplePainEffect);
			m_wSimplePainEffect.SetOpacity(0);
			m_wSimplePainEffect.SetMaskProgress(0);
		};
