...
class ACE_CarriableEntityComponent : ScriptComponent
{
	...
	protected void AttachHandlersAuthority()
	{
		...
		SCR_PlayerController performerPlayerController =
			SCR_PlayerController.Cast(playerManager.GetPlayerController(playerManager.GetPlayerIdFromControlledEntity(m_pCarrier)));
