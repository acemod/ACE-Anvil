...
- **ACE Scopes Dev** for [advanced zeroing](../../components/scopes)
- **ACE Ballistics Dev** for [ballistic tables](../../components/ballistics)
- **ACE Weather Dev** for [Kacestrel 4500](../../components/weather) pocket weather meter

<Image src={`${import.meta.env.BASE_URL}images/sniper/items.png`} alt="items.png" width="768px" height="432px" />

M21, Kacestrel 4500, Vector 21 and ballistic table from left to right.
...
1. Put the Kacestrel 4500 in hand and hold <kbd>R</kbd> to inspect it. A mouse cursor will appear.
...
We can then calculate the interpolation scale by dividing the 39 m by 50 m, which yields **0.78**. We can finally get the data for 539 m by multiplying the 50 m difference data with the interpolation scale and add the 550 m data to it:
...
1. Adjust the DROP and WIND zeroings. For our case `ELEV=3.2` (elevation has the opposite sign of drop) and `WIND=1.8R` (`R` for positive crosswind speed, _i.e._ crosswind component comes from the right).
