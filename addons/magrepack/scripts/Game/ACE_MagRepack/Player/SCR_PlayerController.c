...
{
	//------------------------------------------------------------------------------------------------
	void ACE_MagRepack_Repack(MagazineComponent fromMag, MagazineComponent toMag, SCR_InventoryStorageManagerComponent managerComp,
							  BaseInventoryStorageComponent storageTo, BaseInventoryStorageComponent storageFrom)
	{
		if (!fromMag || !toMag)
		{
		...
		if (fromCount + toCount <= maxCount)
		{
			...
			Print("Added " + (fromCount) + " rounds to toMag. It now holds " + toMag.GetAmmoCount() + ". The remainder in fromMag is " +
					  fromMag.GetAmmoCount() + ".",
				  LogLevel.DEBUG);
		...
		else
		{
			...
			if (remainder > 0)
			{
				...
				if (managerComp.TryRemoveItemFromInventory(fromEntity, storageFrom))
				{
					...
					{
						Print("Was unable to successfully fromMag back into storage. If it could not be droppeed on the ground, it was deleted.",
							  LogLevel.DEBUG);
						if (!managerComp.TryRemoveItemFromInventory(fromEntity))
							SCR_EntityHelper.DeleteEntityAndChildren(fromEntity);
					}
	...
	//------------------------------------------------------------------------------------------------
	void ACE_MagRepack_RequestRepack(MagazineComponent fromMag, MagazineComponent toMag, SCR_InventoryStorageManagerComponent inventoryManager,
									 BaseInventoryStorageComponent toItemStorageComponent, BaseInventoryStorageComponent fromItemStorageComponent)
	{
		Rpc(ACE_MagRepack_RpcAsk_Repack, Replication.FindId(fromMag), Replication.FindId(toMag), Replication.FindId(inventoryManager),
			Replication.FindId(toItemStorageComponent), Replication.FindId(fromItemStorageComponent), );
	}
	...
	protected void ACE_MagRepack_RpcAsk_Repack(RplId fromMag, RplId toMag, RplId managerComp, RplId storageTo, RplId storageFrom)
	{
		...
		ACE_MagRepack_Repack(MagazineComponent.Cast(Replication.FindItem(fromMag)), MagazineComponent.Cast(Replication.FindItem(toMag)),
