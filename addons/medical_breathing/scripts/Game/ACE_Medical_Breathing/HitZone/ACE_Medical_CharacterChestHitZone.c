...
modded class ACE_Medical_CharacterChestHitZone : SCR_CharacterHitZone
{
	...
	override void OnDamage(notnull BaseDamageContext damageContext)
	{
		...
		if (vitals.GetPneumothoraxScale() == 0)
		{
			if (Math.RandomFloat(0, 1) < settings.m_fPneumothoraxChance)
				vitals.SetPneumothoraxScale(settings.m_fPneumothoraxScaleDeteriorationStep);
		}
		else if (!vitals.HasTensionPneumothorax())
		{
			if (Math.RandomFloat(0, 1) < settings.m_fTensionPneumothoraxChance)
