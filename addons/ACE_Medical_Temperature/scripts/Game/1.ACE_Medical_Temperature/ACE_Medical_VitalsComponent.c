modded class ACE_Medical_VitalsComponent : ACE_BaseComponent{
	//Kelvin = 273 + degrees celsius
 	float m_fBodyTemperature;//Core temperature of patient, Kelvin
	float m_fInsulationFactor=0.03;//Insulation factor of clothing - 0.03 = 3% of core heat escapes
	//Returns the core temperature of player in celsius
	float GetTemperature(){
		return body_temperature;
	}
//	void SetTemperature(float m_fNewTemperature){
//		m_fBodyTemperature = m_fNewTemperature;
//	}
	override void Reset(){
		super.Reset();
		...
		if (settings)
		{
			SetTemperature(m_fDefaultCoreTemperature)
		}
	}
}