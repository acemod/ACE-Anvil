...
modded class SCR_ItemPlacementComponent : ScriptComponent
{
	...
	{
		SCR_CampaignBuildingGadgetToolComponent gadgetComponent =
			SCR_CampaignBuildingGadgetToolComponent.Cast(m_PlacedItem.FindComponent(SCR_CampaignBuildingGadgetToolComponent));
		if (!gadgetComponent)
	...
	protected void RpcDo_ACE_Trenches_RequestSpawnTrench(int gadgetID, int variantID, vector transform[4])
	{
		...
		SCR_MultiPartDeployableItemComponent dataComponent =
			SCR_MultiPartDeployableItemComponent.Cast(gadget.FindComponent(SCR_MultiPartDeployableItemComponent));
		if (!dataComponent)
			return;
