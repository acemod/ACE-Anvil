...
class ACE_Medical_SecondChanceSystem : GameSystem
{
	...
	override static void InitInfo(WorldSystemInfo outInfo)
	{
		...
		outInfo.SetAbstract(false).SetUnique(true).SetLocation(WorldSystemLocation.Server).AddPoint(WorldSystemPoint.Frame);
	}
	...
	protected void ApplySecondChanceRegenToHitZone(SCR_CharacterDamageManagerComponent damageManager, SCR_HitZone hitZone)
	{
		...
		SCR_DamageContext regenContext = new SCR_DamageContext(
			EDamageType.REGENERATION, -hitZone.GetMaxHealth() * SECOND_CHANCE_SCALED_RECOVERED_HEALTH, hitPosDirNorm, damageManager.GetOwner(), hitZone,
	...
	void Register(notnull SCR_CharacterDamageManagerComponent damageManager, SCR_CharacterHitZone struckHitZone)
	{
		...
		if (!struckHitZone || !damageManager.ACE_Medical_IsSecondChanceEnabled() || damageManager.ACE_Medical_ShouldDeactivateSecondChance() ||
			!struckHitZone.ACE_Medical_ShouldGrantSecondChance())
		{
