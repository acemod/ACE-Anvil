...
//! ACE_TemplateFrameJobScheduler<SCR_ChimeraCharacter, ACE_Overheating_CharacterContext, ACE_Overheating_AmmoTemperatureJob>
//! as workaround for:
//! - https://feedback.bistudio.com/T193122
//! - https://feedback.bistudio.com/T193389
[BaseContainerProps()]
class ACE_Overheating_AmmoTemperature_JobScheduler : ACE_IFrameJob
{
	...
	protected ref ACE_Overheating_AmmoTemperatureJob m_pTemplateJob;
	protected ref array<ref ACE_Overheating_AmmoTemperatureJob> m_aJobs = {};
	...
	void OnInit(ACE_Overheating_AmmoTemperatureJob templateJob)
	{
	...
	protected void UpdateQueueStep(float currentTime)
	{
		...
		{
			ACE_Overheating_AmmoTemperatureJob job = m_aJobs[i];
	...
	protected void UpdateObjectsStep(float currentTime)
	{
		...
		{
			ACE_Overheating_AmmoTemperatureJob job = m_aJobs[m_iCurrentJobIdx];
			ACE_Overheating_CharacterContext context = job.GetContext();
	...
	void Register(SCR_ChimeraCharacter object)
	{
		...
		ACE_Overheating_CharacterContext context = new ACE_Overheating_CharacterContext(object);
		if (!context.IsValid())
			return;
		
		ACE_Overheating_AmmoTemperatureJob job = CreateJob();
	...
	{
		foreach (int i, ACE_Overheating_AmmoTemperatureJob job : m_aJobs)
		{
	...
	{
		ACE_Overheating_AmmoTemperatureJob job = m_aJobs[machineIdx];
		m_aJobs.Remove(machineIdx);
		job.OnStop();
	}
	
	//------------------------------------------------------------------------------------------------
	protected ACE_Overheating_AmmoTemperatureJob CreateJob()
	{
		ACE_Overheating_AmmoTemperatureJob instance = new ACE_Overheating_AmmoTemperatureJob();
