...
//! ACE_TemplateFrameJobScheduler<SCR_ChimeraCharacter, ACE_Medical_CharacterContext, ACE_Medical_PneumothoraxJob>
//! as workaround for:
//! - https://feedback.bistudio.com/T193122
//! - https://feedback.bistudio.com/T193389
[BaseContainerProps()]
class ACE_Medical_Pneumothorax_JobScheduler : ACE_IFrameJob
{
	...
	protected ref ACE_Medical_PneumothoraxJob m_pTemplateJob;
	protected ref array<ref ACE_Medical_PneumothoraxJob> m_aJobs = {};
	...
	void OnInit(ACE_Medical_PneumothoraxJob templateJob)
	{
	...
	protected void UpdateQueueStep(float currentTime)
	{
		...
		{
			ACE_Medical_PneumothoraxJob job = m_aJobs[i];
	...
	protected void UpdateObjectsStep(float currentTime)
	{
		...
		{
			ACE_Medical_PneumothoraxJob job = m_aJobs[m_iCurrentJobIdx];
	...
	void Register(SCR_ChimeraCharacter object)
	{
		...
		ACE_Medical_PneumothoraxJob job = CreateJob();
	...
	{
		foreach (int i, ACE_Medical_PneumothoraxJob job : m_aJobs)
		{
	...
	{
		ACE_Medical_PneumothoraxJob job = m_aJobs[machineIdx];
		m_aJobs.Remove(machineIdx);
		job.OnStop();
	}
	
	//------------------------------------------------------------------------------------------------
	protected ACE_Medical_PneumothoraxJob CreateJob()
	{
		ACE_Medical_PneumothoraxJob instance = new ACE_Medical_PneumothoraxJob();
